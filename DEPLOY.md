...
Если прокси нет и сервис нужно открыть в доверенной внутренней сети, задайте адрес хоста в `.env`
рядом с `docker-compose.yml` (файл в `.gitignore`) и пересоздайте контейнер:

```bash
echo 'BIND_ADDRESS=192.0.2.10' > .env
docker compose up -d
```

Так трафик идёт без TLS: пароль входа, ключ OpenAI и пароли роутеров при импорте передаются
открытым текстом. Сразу после запуска создайте администратора — до этого форму `/setup` может
заполнить любой, кто видит порт.
